...
data EqualSpacingMsg = MoreSpacing | LessSpacing | DefaultSpacing | NoSpacing deriving (Typeable)
...
instance LayoutModifier EqualSpacing a where
    ...
    pureMess eqsp msg
        ...
        | Just DefaultSpacing <- fromMessage msg = Just $
            eqsp { gap = 36 }

        | Just NoSpacing <- fromMessage msg = Just $
            eqsp { gap = 0 }
